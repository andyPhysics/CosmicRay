...
def gh_function(depth: float, Xo: float, Xmax: float, lambda_value: float) -> float:
    x_normalized = (depth - Xo) / lambda_value
    m = (Xmax - Xo) / lambda_value
    n = ((x_normalized / m) ** m) * np.exp(m - x_normalized)
    return np.log(n)

def process_files(input_file: str) -> dict:
    our_map = {
        'zenith': [],
        'S125': [],
        'energy_loss': [],
    ...
    while l3_file.more():
        ...
        try:
            ...
            # Handle specific exception here if needed
